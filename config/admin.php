...
return [
   ...
   */
    ...
    */
    ...
    /*
    |--------------------------------------------------------------------------
    | Allow unsigned class dispatch
    |--------------------------------------------------------------------------
    |
    | 控制未签名的 action/form 类名是否被允许通过 dcat-api 派发。默认 false，
    | 任何未签名的 _action / _form 请求都会被拒绝。仅在升级期间为兼容老的
    | 浏览器缓存页面时临时设为 true，迁移完成后必须立即关闭，避免攻击者
    | 通过未签名请求直接调用任意 mutating action（如扩展卸载）。
    |
    */
    'allow_unsigned_dispatch' => env('ADMIN_ALLOW_UNSIGNED_DISPATCH', false),
