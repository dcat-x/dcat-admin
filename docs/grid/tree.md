...
> {tip} 此表结构和模型可完全兼容 <code>[模型树](../model-tree/basic.md)</code> 。
...
> {tip} 使用 [查询过滤](filters.md)、[列过滤器](column-filter.md)、[快捷搜索](quick-search.md) 等搜索功能都会<b>取消只查最顶级数据的操作</b>，只有 [筛选器](selector.md) 和 [范围查询scope](filters.md#scope) 等功能例外。
...
[模型树](../model-tree/basic.md)同样可用于展示多层级结构数据，并且支持用拖拽的方式实现数据的层级、排序等操作，但是不支持分页和点击加载功能，只能一次性加载完所有数据，
因此[模型树](../model-tree/basic.md)并不适合用来展示数据量较大的数据。
