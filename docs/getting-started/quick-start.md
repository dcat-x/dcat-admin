...
安装完`Laravel`之后会内置一个`users`表的`migration`文件(如果不了解`migration`文件作用，请参考文档[数据库迁移](https://learnku.com/docs/laravel/12.x/migrations))，文件路径为`database/migrations/2014_10_12_000000_create_users_table.php`。
...
**2.**由于前面已经创建好了数据表，所以这里我们可以直接通过页面左上角的第二个下拉选框选择`users`表，选择之后会自动填充字段信息。

**3.**修改模型名称为`App\Models\User`

**4.**由于`migration`文件、数据表、以及模型文件(使用内置的`App\Models\User`即可)都已经有了，所以此处我们可以把这三个选项给去掉
...
更多详细使用请查看[数据表格](../grid/basic.md)和[数据表单](../form/basic.md)。
...
每个控制器都可以生成自己对应的语言包，并且[数据表格](../grid/basic.md)、[数据表单](../form/basic.md)和[数据详情](../show/basic.md)功能都会自动读取里面的翻译。
